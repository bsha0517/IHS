...
**Additional evidence (P5.5-Z, ZATCA E-Invoicing Sandbox Integration, 2026-09-25):** a full-suite run after this phase's own changes (a new `EInvoiceSubmission` table + a second additive `InvoiceIssued` outbox handler) showed 5 failed assertions across 716 tests, all matching this entry's exact signature — `db.chartOfAccount.findFirstOrThrow()` (outbox-reliability.test.ts), `db.journal.findFirstOrThrow()` (p3-13-cross-role-end-to-end.test.ts, both times), and two "expected 0 admin notifications to be greater than 0" (outbox-crash-recovery.test.ts, outbox-reliability.test.ts) — none referencing ZATCA/e-invoicing anywhere. Confirmed not caused by this phase: the shared org's `zatca.sellerProfile` Setting (the only shared-org state this phase's own test file touches) was verified fully cleaned up post-run (0 rows), and this phase's own dedicated test file (`test/integration/p5-5-z-zatca-einvoicing.test.ts`, which creates and cleans up every fixture it uses) passed deterministically across three consecutive re-runs with no database reset between them. New data point for whoever picks up the "something deletes shared providers" investigation: the missing-row types now confirmed include `chart_of_account` and `journal`, not only `provider` — the deleting code path is broader than originally scoped.
