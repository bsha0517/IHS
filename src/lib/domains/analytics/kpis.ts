...
export async function getKpiTrends(session: SessionContext, months = 6): Promise<MonthlyKpiPoint[]> {
  ...
  const branchWhere = scope.isOrgWide ? {} : { branchId: narrowBranchFilter(scope) }
  const now = new Date()
  ...
  return Promise.all(
    ranges.map(async ({ start, end }) => {
      const [revenue, collections, newPatients, appointmentsByStatus] = await Promise.all([
        db.invoice.aggregate({ where: { organizationId, ...branchWhere, status: { not: "void" }, issuedAt: { gte: start, lt: end } }, _sum: { totalAmount: true } }),
        db.payment.aggregate({ where: { organizationId, ...branchWhere, status: "completed", receivedAt: { gte: start, lt: end } }, _sum: { amount: true } }),
        db.patient.count({ where: { organizationId, createdAt: { gte: start, lt: end } } }),
        db.appointment.groupBy({ by: ["status"], where: { organizationId, ...branchWhere, startTime: { gte: start, lt: end } }, _count: { _all: true } }),
      ])
