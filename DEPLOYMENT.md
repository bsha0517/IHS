...
4. Run `prisma migrate deploy` against the production database as an explicit, separate step — before or during deploy, never automatically on server boot (see Migrations above). This uses `DIRECT_DATABASE_URL`. If this is a re-deploy against an existing database and the migration added tables, re-apply the runtime role's grants (`psql "<owner connection>" -f prisma/db-setup/p0-06-regrant-new-tables.sql`) — step 2 above only granted on whatever tables existed at the time it ran.
5. **Apply and verify Row Level Security** (P4.9.1 — new database provisioning step; see [DATABASE.md](DATABASE.md)'s "Row Level Security" section for the full reasoning): `npm run db:security:apply` then `npm run db:security:check` (must report clean). This must run **after** step 4 (the tables it protects must already exist) and is safe/idempotent to run on every deployment, not only the first. Without this step, the database is reachable via Supabase's own Data API using the project's public anon key, entirely independent of this application's own authentication — see P4_9_1_COMMERCIAL_READINESS_CORRECTIONS_REPORT.md for the real incident this closes.
6. Run `npm run db:seed` once, against the fresh production database, to create the permission catalog, default roles, and the initial Super Admin account. This runs through `DATABASE_URL` (the restricted role) — fine, since seeding is DML (creates/upserts rows), not DDL.
7. **Change the seeded Super Admin password immediately** (`admin@avant.local` / the `prisma/seed.ts`-hardcoded dev password) — this has been a carried-over Next Action since Phase 1 and must happen before any non-local use. There is no forced-password-change-on-first-login flow; this is a manual step.
8. `npm run build` — confirms the production bundle compiles; the same command CI/this build's own verification has run at the end of every phase.
9. Start the app (`npm run start`, or the hosting platform's equivalent).
10. Verify: log in as Super Admin, confirm `/dashboard` renders real data, confirm a representative Server Action (e.g. registering a patient) and the new `/api/reports/export` Route Handler both work against the production database — this exercises `DATABASE_URL`/the restricted role end-to-end, per Database Privileges below.
11. Decide on and configure real SMS/WhatsApp/Email provider credentials only once a specific provider is chosen — see Environments above; nothing reads a provider credential yet, so there's nothing to set until an adapter is actually implemented.
